...
class DouYinCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        if config.ENABLE_IP_PROXY and config.IP_PROXY_POOL_COUNT > 0:
            # 只有在启用代理且代理池数量大于0时才创建代理池
            try:
                ip_proxy_pool = await create_ip_pool(config.IP_PROXY_POOL_COUNT, enable_validate_ip=True)
                ip_proxy_info: IpInfoModel = await ip_proxy_pool.get_proxy()
                playwright_proxy_format, httpx_proxy_format = self.format_proxy_info(ip_proxy_info)
            except Exception as e:
                logger.error(f"创建代理池失败，将使用直接连接: {e}")
                playwright_proxy_format, httpx_proxy_format = None, None
    ...
    def format_proxy_info(ip_proxy_info: IpInfoModel) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        # Firefox对代理格式的要求与Chromium不同，需要特殊处理
        # 参考: https://playwright.dev/python/docs/api/class-browsertype#browser-type-launch-option-proxy
        playwright_proxy = {
            "server": f"{ip_proxy_info.protocol}{ip_proxy_info.ip}:{ip_proxy_info.port}",
            "username": ip_proxy_info.user,
            "password": ip_proxy_info.password,
            "bypass": "<-loopback>"  # 允许本地连接
        }
        httpx_proxy = {
